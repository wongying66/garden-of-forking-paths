...
SCRIPT.discover_aleph = {
    ...
    dialogues: [
        {
            speaker: 'narrator',
            text: '你跟随三条线索来到图书馆。书架上的书越来越薄，最后只剩下一行字：X。'
        },
        {
            speaker: 'narrator',
            text: '回廊尽头是一间圆形房间。房间中央悬浮着一个光点，四周没有门。'
        },
        {
            speaker: 'narrator',
            text: '档案员说：这就是阿莱夫。它不是答案，而是一把能打开其他世界的钥匙。X曾经从这里离开。'
        },
        {
            speaker: 'narrator',
            text: '你走近光点。光芒里浮出三扇门，每扇门后都传来不同的声音。要继续追踪 X，你必须选择一扇门。',
            showChoices: true
        }
    ],
    choices: [
        { text: '触碰阿莱夫，查看三扇门', next: 'aleph_revelation' }
    ]
};
...
SCRIPT.aleph_revelation = {
    ...
    dialogues: [
        {
            speaker: 'narrator',
            text: '光点展开成一座门廊。门后不是一条固定路线，而是一个个可以进入、完成、再返回的世界。'
        },
        {
            speaker: 'narrator',
            text: '你短暂看见几幅画面：雨里的霓虹、海底的神庙、正在下坠的少女、燃烧的戒指。它们不是答案，只是入口。'
        },
        {
            speaker: 'narrator',
            text: '每个世界都可能藏着一片 X 的线索。你需要在世界里完成一个任务，带着线索回来，再决定下一扇门。'
        },
        {
            speaker: 'narrator',
            text: '阿莱夫的规则很简单：进入一扇门，完成它；回到这里，选择下一扇。你走过的世界会被记录，选择也会留下后果。',
            showChoices: true
        },
    ],
    choices: [
...
SCRIPT.aleph_understanding = {
    ...
    dialogues: [
        {
            speaker: 'narrator',
            text: '一个声音从门廊深处传来：阿莱夫不是目的地，是方法。你要用它进入不同世界，再把线索带回来。'
        },
        {
            speaker: 'narrator',
            text: '每个世界都会根据你的选择回应你。有些选择马上改变场景，有些会在回来以后才显出结果。'
        },
        {
            speaker: 'narrator',
            text: '先记住三件事：完成世界、留意 X 的线索、回到阿莱夫继续选择。现在，去看看哪些门已经打开。',
            showChoices: true
        }
    ],
    choices: [
...
SCRIPT.aleph_choice = {
    ...
    getDialogues: () => {
        ...
        let text = `阿莱夫把当前开放的门排列在你面前。${getEndingProgressText()}。`;
        
        if (completed.length > 0) {
            text += `\n你已经完成 ${completed.length} 个世界，收集了 ${totalClues} 条关于 X 的线索。走过的世界会保留记录，也可以重新进入。`;
        }
        ...
        if (xHiddenUnlocked) {
            text += '\n\n一面金色的镜子浮现出来。那不是普通的世界，而是终章之后才会打开的入口。';
        } else if (incomplete.length > 0) {
            text += '\n\n选择一扇门进入世界。完成任务后，你会回到这里继续追踪 X。';
        } else if (totalClues >= FIRST_RUN_ENDING_THRESHOLD) {
            text += '\n\n当前开放的世界都已完成。终章已经可以进入。';
        } else {
            text += '\n\n当前开放的世界都已完成，但线索还不够。你可以重新进入世界，寻找遗漏的线索。';
        }
